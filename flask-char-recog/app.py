...
def save_to_database(response):
    ...
    cursor.execute('''
        INSERT INTO response_data (nama, alamat, agama, kecamatan, nik, ttl, jenis_kelamin, status_perkawinan, pekerjaan, kewarganegaraan, berlaku_hingga)
        VALUES (?, ?, ?, ?,?, ?, ?, ?,?,?,? )
    ''', (response['nama'], response['alamat'], response['agama'], response['kecamatan'], response['nik'], response['ttl'], response['jenis_kelamin'], response['status_perkawinan'], response['pekerjaan'], response['kewarganegaraan'], response['berlaku_hingga']))
...
def retrieve_data_from_database():
    ...
    data = [{'nama': row[1], 'alamat': row[2], 'agama': row[3], 'kecamatan': row[4], 'nik' : row[5], 'ttl' : row[6], 'jenis_kelamin' : row[7], 'status_perkawinan' : row[8], 'pekerjaan' : row[9], 'kewarganegaraan' : row[10], 'berlaku_hingga' : row[11] } for row in rows]
...
def upload():
    ...
    nama, alamat, agama, kecamatan, nik, ttl, jenis_kelamin, status_perkawinan, pekerjaan, kewarganegaraan, berlaku_hingga = KTPOCR.process_image(temp_image_path)
